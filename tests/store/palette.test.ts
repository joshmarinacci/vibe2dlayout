...
function addRect(state: AppState, overrides?: Partial<{ fillColor: string; fillPaletteId: string; strokeColor: string; strokePaletteId: string }>) {
  const shape = createShape('rect', 100, 100)
  if (overrides?.fillColor) {
    (shape as any).fill = { ...(shape as any).fill, color: overrides.fillColor, paletteColorId: overrides.fillPaletteId }
  }
  if (overrides?.strokeColor) {
    (shape as any).stroke = { ...(shape as any).stroke, color: overrides.strokeColor, paletteColorId: overrides.strokePaletteId }
  }
...
describe('ADD_PALETTE_COLOR', () => {
  it('adds a color to the palette', () => {
    let s = appReducer(initialState, { type: 'ADD_PALETTE', palette: testPalette })
    s = appReducer(s, { type: 'ADD_PALETTE_COLOR', paletteId: 'p1', color: { id: 'c3', name: 'Green', color: '#00ff00' } })
...
describe('UPDATE_PALETTE_COLOR', () => {
  it('updates the palette color hex', () => {
    let s = appReducer(initialState, { type: 'ADD_PALETTE', palette: testPalette })
    s = appReducer(s, { type: 'UPDATE_PALETTE_COLOR', paletteId: 'p1', colorId: 'c1', color: '#ff8800' })
    const palette = s.document.palettes.find(p => p.id === 'p1')!
  ...
  it('name-only update does not change shape colors', () => {
    ...
    const r = addRect(s, { fillColor: '#ff0000', fillPaletteId: 'c1' })
    s = r.state
    s = appReducer(s, { type: 'UPDATE_PALETTE_COLOR', paletteId: 'p1', colorId: 'c1', name: 'Renamed Red' })
    const shape = s.document.shapes[r.id] as any
    expect(shape.fill.color).toBe('#ff0000')  // unchanged
    const palette = s.document.palettes.find(p => p.id === 'p1')!
...
describe('UPDATE_PALETTE_COLOR undo', () => {
  it('undo restores both palette and shape colors', () => {
    ...
    h = historyReducer(h, { type: 'UPDATE_PALETTE_COLOR', paletteId: 'p1', colorId: 'c1', color: '#ff8800' })
