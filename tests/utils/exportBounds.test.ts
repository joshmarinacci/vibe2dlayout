...
import { applyTransform, shapeCorners, computeVisualBounds } from '../../src/utils/exportBounds'
...
function makeText(id: string, t: Partial<BoundingBox> & { x: number; y: number; width: number; height: number }): Shape {
  return {
    id,
    type: 'text',
    visible: true,
    ...
    text: { content: 'test', fontSize: 14, fontFamily: 'sans-serif', fontWeight: 'normal', fontStyle: 'normal', textAlign: 'left', color: '#000', lineHeight: 1.4, letterSpacing: 0, textDecoration: 'none', smallCaps: false },
...
describe('computeVisualBounds', () => {
  ...
  it('ignores line shapes (no transform)', () => {
    const line = { id: 'l', type: 'line', visible: true, locked: false, opacity: 1, boxShadow: [] } as unknown as Shape
