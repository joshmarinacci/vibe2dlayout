...
import { setPixel, hexToRgba } from '@model/pixelAsset'
...
function bresenham(x0: number, y0: number, x1: number, y1: number): Array<{ col: number; row: number }> {
...
function redraw(canvas: HTMLCanvasElement, asset: PixelAsset, pixels: number[], zoom: number) {
  ...
  ctx.lineWidth = 0.5
  for (let col = 0; col <= asset.width; col++) {
    ctx.beginPath(); ctx.moveTo(col * zoom, 0); ctx.lineTo(col * zoom, h); ctx.stroke()
  }
  for (let row = 0; row <= asset.height; row++) {
    ctx.beginPath(); ctx.moveTo(0, row * zoom); ctx.lineTo(w, row * zoom); ctx.stroke()
  }
}

function getPixelCoords(canvas: HTMLCanvasElement, clientX: number, clientY: number, zoom: number, asset: PixelAsset): { col: number; row: number } | null {
...
export function PixelEditorOverlay({ asset, palettes, dispatch }: Props) {
  ...
  for (const palette of palettes) {
    for (const c of palette.colors) {
      if (!seen.has(c.color)) { seen.add(c.color); allColors.push(c.color) }
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {allColors.map(color => (
          <div
            ...
            title={color}
            style={{
              width: 18, height: 18, borderRadius: 3, background: color, cursor: 'pointer', flexShrink: 0,
      ...
      <div style={{
        ...
        color: 'rgba(255,255,255,0.6)', fontSize: 11, userSelect: 'none', pointerEvents: 'none',
      }}>
