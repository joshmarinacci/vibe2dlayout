...
export function ChartMockShapeComp({shape, isSelected, onClick, onDoubleClick, handDrawn}: Props) {
    ...
    const axisColor = strokeColor(stroke)
    ...
    return (
        <BoxShapeBase shape={shape} isSelected={isSelected} onClick={onClick}
            ...
            {handDrawn ? (
                ...
                >
                    {/* Axes */}
                    <line x1={axisX} y1={p} x2={axisX} y2={axisY} stroke={axisColor}
                          strokeWidth={stroke.width}/>
                    <line x1={axisX} y1={axisY} x2={w - p} y2={axisY} stroke={axisColor}
                          strokeWidth={stroke.width}/>

                    {chartType === 'bar' ? (
                        bars.map((b, i) => (
                            <rect key={i} x={b.bx} y={b.by} width={b.bw} height={b.bh}
                                  fill={barColor}/>
                        ))
                    ) : (
                        <>
