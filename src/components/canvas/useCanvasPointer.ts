...
export function useCanvasPointer(containerRef: RefObject<HTMLDivElement | null>) {
    ...
    const onPointerDown = useCallback((e: React.PointerEvent) => {
        ...
        if (hitId) {
            ...
            if (!alreadySelected) {
                ...
                dispatch({
                    type: 'SELECT_SHAPES',
                    ids: state.selection.ids.filter(id => id !== hitId),
                    additive: false
                })
            }
    ...
    const onPointerMove = useCallback((e: React.PointerEvent) => {
        ...
        if (guideCreating.current) {
            ...
            setGuidePreview({
                id: '__preview__',
                orientation: guideCreating.current.orientation,
                position: newPos
            })
            return
        }
    ...
    const onPointerUp = useCallback((e: React.PointerEvent) => {
        ...
        if (guideCreating.current && state.activePageId) {
            dispatch({
                type: 'ADD_GUIDE', pageId: state.activePageId,
        ...
        if (shapeType && isDragging.current) {
            ...
            if (shapeType === 'line') {
                ...
                if (shape.type !== 'line') {
                    const newShape = {
                        ...shape,
                        transform: {...shape.transform, x: lx, y: ly, width: sw, height: sh}
                    }
                    dispatch({type: 'ADD_SHAPE', parentId: drillParentId, shape: newShape})
                    dispatch({type: 'SELECT_SHAPES', ids: [newShape.id], additive: false})
                    if (shapeType === 'page') dispatch({
