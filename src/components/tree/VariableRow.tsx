...
export function VariableRow({ variable, isSelected, isFirst, isLast, dispatch }: Props) {
  ...
  const contextGroups: ContextMenuGroup[] = [
    {
      items: [
        { label: 'Rename', onClick: () => { setEditName(variable.name); setIsEditingName(true) } },
        { label: 'Move Up', onClick: () => dispatch({ type: 'REORDER_VARIABLE', variableId: variable.id, direction: 'up' }), disabled: isFirst },
        { label: 'Move Down', onClick: () => dispatch({ type: 'REORDER_VARIABLE', variableId: variable.id, direction: 'down' }), disabled: isLast },
      ],
    },
    {
      items: [
  ...
  return (
    <div
      ...
      onDoubleClick={() => { setEditName(variable.name); setIsEditingName(true) }}
