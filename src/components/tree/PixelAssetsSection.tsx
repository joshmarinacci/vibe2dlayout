...
export function PixelAssetsSection({assets, selectedPixelAssetId, dispatch, shapes}: Props) {
    ...
    const usageCount = (assetId: string) =>
        Object.values(shapes).filter(s => s.type === 'pixelimage' && (s as {
            assetId?: string
        ...
        <div>
            <div className={sectionStyles.header}>
                <div className={sectionStyles.headerLabel} onClick={() => setCollapsed(v => !v)}>
                    <span
                        className={`${sectionStyles.chevron} ${collapsed ? '' : sectionStyles.chevronOpen}`}>›</span>
            ...
            {!collapsed && assets.map(asset => (
              ...
              ×{usageCount(asset.id)}
                    ...
                    <button
                        ...
                        onClick={e => {
                            e.stopPropagation();
                            dispatch({type: 'DELETE_PIXEL_ASSET', assetId: asset.id})
                        }}
