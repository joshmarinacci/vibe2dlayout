...
export function StyleRow({ style, isSelected, isFirst, isLast, dispatch }: Props) {
  ...
  const contextGroups: ContextMenuGroup[] = [
    {
      items: [
        { label: 'Rename', onClick: () => { setEditName(style.name); setIsEditingName(true) } },
        { label: 'Move Up', onClick: () => dispatch({ type: 'REORDER_TEXT_STYLE', styleId: style.id, direction: 'up' }), disabled: isFirst },
        { label: 'Move Down', onClick: () => dispatch({ type: 'REORDER_TEXT_STYLE', styleId: style.id, direction: 'down' }), disabled: isLast },
      ],
    },
    {
      items: [
  ...
  return (
    <div
      ...
      onDoubleClick={() => { setEditName(style.name); setIsEditingName(true) }}
