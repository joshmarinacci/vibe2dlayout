...
export function AssetsSection({ assets, selectedAssetId, dispatch, shapes }: Props) {
  ...
  const usageCount = (assetId: string) =>
    Object.values(shapes).filter(s => s.type === 'image' && (s as { assetId?: string }).assetId === assetId).length
            ...
            <div className={sectionStyles.addUrlMenu}>
              ...
              <div className={sectionStyles.addUrlField}>
                ...
                <input
                  ...
                  placeholder="https://..."
                  onKeyDown={e => { if (e.key === 'Enter') addUrlAsset() }}
                  autoFocus
                />
              </div>
              <div className={sectionStyles.addUrlActions}>
                <button className={sectionStyles.addUrlCancel} onClick={() => setShowAddMenu(false)}>Cancel</button>
                <button className={sectionStyles.addUrlConfirm} onClick={addUrlAsset}>Add</button>
              </div>
            </div>
          )}
        </div>
