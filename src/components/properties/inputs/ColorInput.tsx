...
export function ColorInput({ label, value, onChange, variableId, variables, onVariableChange }: Props) {
      ...
      <div className={styles.inputRow} ref={wrapRef} style={{ position: 'relative' }}>
        ...
        {hasVars && onVariableChange && !showDropdown && (
          <button
            ...
            onClick={() => { setShowDropdown(true); setVarQuery('') }}
          >@</button>
        )}
        {showDropdown && (
              ...
              filteredVars.map(v => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div className={styles.varColorSwatch} style={{ background: String(v.value) }} />
                    <span>@{v.name}</span>
                  </div>
