...
export function ToggleInput({ label, value, onChange, variableId, variables, onVariableChange }: Props) {
  ...
  return (
    <div className={styles.field}>
      <label className={styles.label}>{label}</label>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, position: 'relative' }} ref={wrapRef}>
        <input
          type="checkbox"
          checked={value}
        ...
        {showDropdown && variables && onVariableChange && (
          <div className={styles.varDropdown} style={{ top: 'calc(100% + 4px)', left: 0, minWidth: 140 }}>
            {variables.length === 0 ? (
