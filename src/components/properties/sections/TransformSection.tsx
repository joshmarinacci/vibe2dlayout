    ...
    return (
      <div className={styles.tfield}>
        ...
        <div className={inputStyles.varBound} style={{ flex: 1 }}>
          ...
          <button className={inputStyles.varClear} onClick={() => onVariableChange(null)} title="Clear variable binding">×</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={styles.tfield} ref={wrapRef} style={{ position: 'relative' }}>
      ...
      <input
        ...
        onKeyDown={e => {
          if (e.key === 'Enter') {
            ...
            if (!isNaN(v)) { const next = v + 1; setLocalText(String(next)); onChange(min !== undefined ? Math.max(min, next) : next) }
      ...
      {showDropdown && onVariableChange && (
        <div className={inputStyles.varDropdown} style={{ top: 'calc(100% + 2px)', left: 0, right: 0, minWidth: 120 }}>
          {filteredVars.length === 0 ? (
            ...
            filteredVars.map(v => (
              ...
              >
                <span>@{v.name}</span>
                <span className={inputStyles.varDropdownVal}>{String(v.value)}</span>
              </div>
            ))
          )}
        </div>
