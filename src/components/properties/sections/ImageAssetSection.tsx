  ...
  return (
    <>
      <div className={styles.section}>
        <div className={styles.sectionTitle}>Image Asset</div>
        <div className={styles.nameRow}>
          <input
            className={styles.nameInput}
            value={asset.name}
            onChange={e => dispatch({ type: 'UPDATE_IMAGE_ASSET', asset: { ...asset, name: e.target.value } })}
        ...
        {isEmbedded ? (
          <>
            ...
            <div className={styles.row}>
              ...
              <span style={{ fontSize: 12, color: '#555' }}>{formatBytes(asset.src)}</span>
            </div>
            {(asset.width !== undefined && asset.height !== undefined) && (
              <div className={styles.row}>
                <label className={styles.label}>Dimensions</label>
                <span style={{ fontSize: 12, color: '#555' }}>{asset.width} × {asset.height} px</span>
              </div>
            )}
          </>
        ) : (
          <>
            <div className={styles.row}>
              ...
              <input
                ...
                onBlur={commitUrl}
                onKeyDown={e => { if (e.key === 'Enter') commitUrl(); if (e.key === 'Escape') setUrlDraft(null) }}
                placeholder="https://..."
              />
            </div>
            {(asset.width !== undefined && asset.height !== undefined) && (
      ...
      <div className={styles.section}>
        ...
        {usedByShapes.length > MAX_SHAPES_SHOWN && (
          <div className={styles.row} style={{ paddingLeft: 8 }}>
            <span style={{ fontSize: 11, color: '#aaa' }}>…and {usedByShapes.length - MAX_SHAPES_SHOWN} more</span>
          </div>
        )}
      </div>
    </>
