...
import {fetchLospecPalette, parseCoolorsUrl, parseGPL, parseHexFile} from '@utils/paletteImport'
import {Link, Plus, Trash2, Upload, X} from 'lucide-react'
...
export function PaletteEditorModal({onClose}: Props) {
    ...
    const importPaletteFromUrl = async () => {
        ...
        try {
            ...
            if (coolorsColors) {
                addImportedPalette({
                    name: 'Coolors Palette',
                    colors: coolorsColors.map(hex => ({name: hex, hex}))
                })
            } else {
                        ...
                        <div className={styles.importSection}>
                            ...
                            <div className={styles.importUrlRow}>
                                <input
                                    ...
                                    onKeyDown={e => {
                                        if (e.key === 'Enter') importPaletteFromUrl()
                                    }}
                                />
                                <button
