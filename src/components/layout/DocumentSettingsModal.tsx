      ...
      <div className={styles.modal} onClick={e => e.stopPropagation()}>
        ...
        <div className={styles.body}>
          ...
          <div className={styles.row}>
            ...
            <div className={styles.control}>
              <input
                ...
                onChange={e => dispatch({ type: 'UPDATE_GRID_SETTINGS', patch: { snapEnabled: e.target.checked } })}
              />
            ...
            <div className={styles.control}>
              <input
                ...
                onChange={e => {
                  ...
                  if (v >= 1 && v <= 200) dispatch({ type: 'UPDATE_GRID_SETTINGS', patch: { size: v } })
                }}
                style={{ width: 72, fontSize: 13, padding: '3px 6px', border: '1px solid #ccc', borderRadius: 4, outline: 'none' }}
              />
          ...
          <div className={styles.row}>
            <label className={styles.label}>
              Grid Style
              <span className={styles.hint}>How the grid is displayed on the canvas</span>
            </label>
            <div className={styles.control}>
              {(['lines', 'dots', 'none'] as GridStyle[]).map(style => (
                <label key={style} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: '#444', cursor: 'pointer' }}>
                  <input
                    type="radio"
                    name="gridStyle"
                    value={style}
                    checked={grid.style === style}
                    onChange={() => dispatch({ type: 'UPDATE_GRID_SETTINGS', patch: { style } })}
                  />
        ...
        <div className={styles.footer}>
          <button
            ...
            onClick={() => dispatch({ type: 'UPDATE_GRID_SETTINGS', patch: { ...DEFAULT_GRID_SETTINGS } })}
          >
            <RotateCcw size={13} />
            Reset to defaults
          </button>
